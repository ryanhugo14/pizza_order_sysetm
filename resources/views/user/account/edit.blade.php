...
@section('content')
    <div class="flex flex-col md:flex-row space-y-10 md:space-x-8 relative p-3 lg:p-10">

        <a href="{{ route('user#detailPage') }}" class="absolute left-2 md:left-6 lg:left-20 top-5 flex items-center">
            <i class="fa-solid fa-left-long text-xl mr-2"></i>
            <span class="font-semibold">Back</span>
        </a>
        {{-- left --}}
        <div class="p-8 bg-white shadow-md text-center md:w-[35%] lg:w-[30%] h-[450px] ">
            <h1 class="text-3xl font-bold capitalize mb-2">{{ $profile->name }}</h1>
            <h3 class="text-md font-semibold mb-8">{{ $profile->email }}</h3>
            ...
            <form action="{{ route('user#upload',Auth::user()->id) }}" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="mb-4">
                    <input type="file" name="image" id="image"
                        class="block mx-auto file:py-1 file:px-2 file:rounded-md file:border-0 file:bg-white file:text-sm file:font-semibold px-3 py-2 border w-72 md:w-full text-sm rounded-lg bg-slate-200 focus:outline-none ring-0 @error('image') is-invalid @enderror ">
                         @error('image')
                            <small class="text-[#ff0000]">{{ $message }}</small>
                        @enderror
                </div>
                <button class="w-40 md:w-full px-3 py-2 bg-yellow-500 font-semibold text-white rounded-lg"
                    type="submit">Upload
                    Photo</button>
            </form>
            <h4 class="text-slate-500 text-xs mt-10">Member Since: <br class=" hidden md:inline-block lg:hidden"> <span
                    class="text-slate-600 font-bold ml-2">{{ $profile->created_at->format('j F Y') }}</span></h4>
        </div>
        {{-- right --}}
        <div class=" bg-slate-50 text-start md:w-[65%] lg:w-[70%] shadow-md">
            <div class="p-8 pb-0">
            ...
            <form action="{{ route('user#update') }}" method="get">
                ...
                <div class="bg-white w-full p-8 grid grid-cols-1 md:grid-cols-2 gap-5">
                    <input type="text" name="id" id="id" value="{{ $profile->id }}" class="hidden">
                    <div>
                    ...
                    <div>
                        ...
                        <select name="gender" id="gender"
                            class="w-full font-semibold text-black text-sm  py-3 px-5 mt-3 placeholder:text-sm border border-slate-200 rounded-md focus:ring-0  focus:border-yellow-500 @error('gender') is-invalid @enderror">
                    ...
                    <div>
                        ...
                        <input type="text" name="role" id="role" value="{{ $profile->role }}"
                            class="w-full font-bold  text-black text-sm  py-3 px-5 mt-3 placeholder:text-sm border border-slate-200 rounded-md focus:ring-0  focus:border-yellow-500 disabled:bg-slate-200/70 disabled:text-slate-500"
                            disabled>
                    </div>
                </div>
                <div class="p-8 pt-0 flex justify-end">
                    <button type="submit" class="px-3 py-2 w-40 bg-yellow-500 text-white rounded-md font-semibold">Update
                        info</button>
                </div>
            </form>
        </div>
